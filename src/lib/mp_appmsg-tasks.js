...
const publishAppmsg = async (cookies, token, send_time, is_release_publish_page, reprint_info, appmsgid, appmsg_item_count) => {
  const opts = {
  ...
  let res = (await netFetch(api.operation_seq(token), { ...opts, body: formdata })).json()
  ...
  if (reprint_info) {
    reprint_confirm = 1
    reprint_info_json_str = JSON.stringify(reprint_info)
  }
  let isMulti = appmsg_item_count > 1 ? 1 : 0

  formdata = `token=${token}&lang=zh_CN&f=json&ajax=1&fingerprint=aa8c8d5bea554e6e4c9ea1ed0dbb354e&random=${Math.random()}&ack=&code=&reprint_info=${reprint_info_json_str}&reprint_confirm=${reprint_confirm}&list=&groupid=&sex=0&country=&province=&city=&send_time=${send_time}&type=10&share_page=1&synctxweibo=0&operation_seq=${operation_seq}&req_id=${req_id}&req_time=${req_time}&sync_version=1&isFreePublish=false&appmsgid=${appmsgid}&isMulti=${isMulti}&direct_send=1`
  if (send_time > 0) {  
    res = (await netFetch(api.timing_publish(token, is_release_publish_page), { ...opts, body: formdata })).json()
  } else {
    res = (await netFetch(api.instant_push(token, is_release_publish_page), { ...opts, body: formdata })).json()
  }
