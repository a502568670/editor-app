...
const handleAppMsgDelete = async () => {
  if (!selectedAccountRef.value) {
    return
  }
  if (!validateAccount()) {
  ...
  ElMessageBox.confirm(
    `会删除草稿箱中前${delete_count}条草稿，删除后不可找回，确定批量删除草稿吗?`,
    '提示',
    {
...
const registerChannels = () => {
  channelCleans[channelName] = window.ipcRenderer.receive(channelName, (msg) => {
    if (typeof msg === 'object' && Object.prototype.hasOwnProperty.call(msg, 'tag')) {
      ...
      if (tag === "appmsg-ret:listAppmsgsInDraftBox") {
      ...
      } else if (tag === "appmsg-ret:deleteDraft") {
        console.log("material_lib deleteDraftResult msg.data=>", msg.data)
        dataLoadingRef.value = false
        const { source, ret } = msg.data
        if (source !== channelSource) {
          return
        ...
        if (success) {
          ...
          ElMessageBox.alert(`批量删除草稿出现错误:${retmsg}`, '错误', {
            confirmButtonText: '确定',
            type: 'error'
          }).catch(() => {
            console.log("batch delete receive catch")
          })
        }
      }
    }
